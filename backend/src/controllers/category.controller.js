...
const getCategoryData = async (req, res, next) => {
  try {
    ...
    // ensure soft-delete column exists and execute queries with retry on missing column
    const ensureIsDeletedExists = async () => {
      try {
        await pool.query('ALTER TABLE categories ADD COLUMN is_deleted TINYINT(1) NOT NULL DEFAULT 0');
      } catch (e) {
        // ignore errors (column may already exist or no permission)
      }
    };
    ...
    const execQuery = async (sql, params) => {
      try {
        ...
        if (err && (err.code === 'ER_BAD_FIELD_ERROR' || /is_deleted/.test(err.message))) {
          await ensureIsDeletedExists();
          return await pool.query(sql, params);
        }
        throw err;
      }
    ...
    const [categories] = await execQuery(
    ...
    const [allTransactions] = await execQuery(
    ...
    const categoriesWithPercent = categories.map((c) => {
      ...
      const userOwned = !isSystem && c.user_id === userId;
      return {
...
const deleteCategory = async (req, res, next) => {
  try {
    ...
    if (category.user_id === null) {
      return error(res, 'Không thể xoá danh mục hệ thống', 403);
    }
    ...
    // Soft-delete: mark as deleted so transactions remain but category is hidden
    const [result] = await pool.query(
      'UPDATE categories SET is_deleted = 1 WHERE category_id = ? AND user_id = ?',
      [categoryId, userId]
    );

    if (result.affectedRows === 0) {
      return error(res, 'Không thể xoá danh mục', 400);
    }
