...
export const categoryService = {
   ...
   * Tạo danh mục mới
   */
  createCategory: async (payload: { name: string; type: 'expense' | 'income' }) => {
    const response = await apiClient.post<unknown, unknown>('/categories', payload);
    return response;
  },
  
  deleteCategory: async (categoryId: number) => {
    const response = await apiClient.delete<unknown, unknown>(`/categories/${categoryId}`);
    return response;
  },
};