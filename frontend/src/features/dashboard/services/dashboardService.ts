...
export const dashboardService = {
  getDashboardSummary: async (): Promise<DashboardSummary> => {
    ...
    const incomeTransactions = transactions.filter(
      (t) => String(t.type).toUpperCase() === "INCOME"
    );
    const incomeCategoryMap = new Map<string, number>();
    incomeTransactions.forEach((t) => {
      const catName = t.category_name || "Khác";
      const amt = Number(t.amount) || 0;
      incomeCategoryMap.set(catName, (incomeCategoryMap.get(catName) || 0) + amt);
    });

    const incomeCategoryList = Array.from(incomeCategoryMap.entries()).map(([name, val]) => ({
      name,
      amount: val,
    }));
    incomeCategoryList.sort((a, b) => b.amount - a.amount);

    const categoryIncome: CategorySpend[] = incomeCategoryList.slice(0, 5).map((item, index) => ({
      name: item.name,
      amount: item.amount,
      percentage: totalIncome > 0 ? Math.round((item.amount / totalIncome) * 100 * 10) / 10 : 0,
      color: colors[index % colors.length],
    }));
